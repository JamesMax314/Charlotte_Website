...
| 23    | Type sizes are chosen from a list, not typed into a spinner. One `<select>` of point sizes — bounded to a scrolling box with `base-select`, since a native popup's height is the browser's to decide — serves both the box and the run, and a span's size style is rebased against what it nests inside so the editor finally paints the size it reports         |
