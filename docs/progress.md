...
| 30    | An alignment grid over every free-form wall, switched on in page settings and spaced in columns across the width. Its lines are snap targets in their own right, merged with the edge guides so the nearer of the two wins. Editor only — no visitor ever sees it. Each line is a memoised div filled with a repeating gradient: an SVG of percentage coordinates rendered in Chrome alone, and dashed borders down a canvas thousands of pixels tall cost tens of thousands of dash segments to paint |
